...
public class RuleParser {
    ...
    public static void main(String args[]) {
        ...
        try {
            RuleParser ruleParser = new RuleParser(args[0], null);
    ...
    public RuleParser(String path, PmEngine engine) {
        myScanner = new RuleScanner(path);
        myEngine = engine;
    }

    public RuleParser(File inputFile, PmEngine engine) {
    ...
    public String parse() {
        ...
        if (result != null) {
            if (myEngine != null) {
               ...
               // myEngine.deleteScriptInternal(scriptId);
            }
        }
        return result;
    }
    ...
    private void semopScript() {
        ...
        if (myEngine != null) {
            myEngine.createScriptRecord(scriptId, scriptName);
        }
    }
    ...
    private void semopUpdateType() {
        ...
        if (defaultType.equals(PmEngine.PM_NODE_POL)
                || defaultType.equals(PmEngine.PM_NODE_CONN)
                || defaultType.equals(PmEngine.PM_NODE_USER)
                || defaultType.equals(PmEngine.PM_NODE_UATTR)) {
            if (crtToken.tokenId == RuleScanner.PM_USER) {
                defaultType = PmEngine.PM_NODE_USER;
            } else if (crtToken.tokenId == RuleScanner.PM_OBJECT) {
                defaultType = PmEngine.PM_OBJ;
                ...
                || defaultType.equals(PmEngine.PM_NODE_OATTR)) {
            if (crtToken.tokenId == RuleScanner.PM_USER) {
                defaultType = PmEngine.PM_NODE_USER;
            } else if (crtToken.tokenId == RuleScanner.PM_OBJECT) {
                defaultType = PmEngine.PM_OBJ;
    ...
    private void semopAUser() {
        ...
        //  PmEngine.PM_NODE_USER + "\")");
        //traceSemact("store userSpec into patternSpec.userSpecs");
        UserSpec userSpec = new UserSpec(crtToken.tokenValue, PmEngine.PM_NODE_USER);
    ...
    private void semopAnActiveUattr() {
        ...
        //traceSemact("create userSpec(\"" + crtToken.tokenValue + "\", \"" +
        //  PmEngine.PM_NODE_AUATTR + "\")");
        //traceSemact("store userSpec into hsUserSpecs");
        UserSpec userSpec = new UserSpec(crtToken.tokenValue, PmEngine.PM_NODE_AUATTR);
        patternSpec.getUserSpecs().add(userSpec);
    }
    ...
    private void semopAUattr() {
        ...
        //traceSemact("create userSpec(\"" + crtToken.tokenValue + "\", \"" +
        //  PmEngine.PM_NODE_UATTR + "\")");
        //traceSemact("store userSpec into hsUserSpecs");
        UserSpec userSpec = new UserSpec(crtToken.tokenValue, PmEngine.PM_NODE_UATTR);
        patternSpec.getUserSpecs().add(userSpec);
    }
    ...
    private void semopASession() {
        ...
        //  PmEngine.PM_SESSION + "\")");
        //traceSemact("store userSpec into hsUserSpecs");
        UserSpec userSpec = new UserSpec(PmEngine.PM_SESSION);
    ...
    private void semopAProcess() {
        ...
        //  PmEngine.PM_PROCESS + "\")");
        //traceSemact("store userSpec into hsUserSpecs");
        UserSpec userSpec = new UserSpec(crtToken.tokenValue, PmEngine.PM_PROCESS);
        userSpec.setId(crtToken.tokenValue);
        patternSpec.getUserSpecs().add(userSpec);
    }
    ...
    private void semopAPc() {
        ...
        //  PmEngine.PM_NODE_POL + "\")");
        //traceSemact("store pcSpec into hsPcSpecs");
        PcSpec pcSpec = new PcSpec(crtToken.tokenValue, PmEngine.PM_NODE_POL);
    ...
    private void semopAnOp() {
        ...
        OpSpec opSpec = new OpSpec(crtToken.tokenValue, PmEngine.PM_OP);
    ...
    private void semopObj() {
        ...
        ObjSpec objSpec = new ObjSpec(crtToken.tokenValue, PmEngine.PM_OBJ);
    ...
    private void semopObjAsCont() {
        ...
        ContSpec contSpec = new ContSpec(crtToken.tokenValue, PmEngine.PM_OBJ);
    ...
    private void semopAnOattr() {
        ...
        traceSemact("create contSpec(\"" + crtToken.tokenValue + "\", \""
                + PmEngine.PM_NODE_OATTR + "\")");
        traceSemact("save contSpec to pattern's contSpecs");
        ContSpec contSpec = new ContSpec(crtToken.tokenValue, PmEngine.PM_NODE_OATTR);
        patternSpec.getContSpecs().add(contSpec);
    }
    ...
    private void semopARecord() {
        ...
        traceSemact("create contSpec(\"" + crtToken.tokenValue + "\", \""
                + PmEngine.PM_RECORD + "\")");
        traceSemact("save contSpec to pattern's contSpecs");
        ContSpec contSpec = new ContSpec(crtToken.tokenValue, PmEngine.PM_RECORD);
        patternSpec.getContSpecs().add(contSpec);
    }
    ...
    private void semopAnyRecord() {
        ...
        traceSemact("create contSpec(\"" + "*" + "\", \""
                + PmEngine.PM_RECORD + "\")");
        traceSemact("save contSpec to pattern's contSpecs");
        ContSpec contSpec = new ContSpec("*", PmEngine.PM_RECORD);
        patternSpec.getContSpecs().add(contSpec);
    }
    ...
    private void semopAClass() {
        ...
        traceSemact("create contSpec(\"" + crtToken.tokenValue + "\", \""
                + PmEngine.PM_OBJ_CLASS + "\")");
        traceSemact("save contSpec to pattern's contSpecs");
        ContSpec contSpec = new ContSpec(crtToken.tokenValue, PmEngine.PM_OBJ_CLASS);
        patternSpec.getContSpecs().add(contSpec);
    }
    ...
    private void semopGrantedOp() {
        ...
        traceSemact("create opndSpec(\"" + PmEngine.PM_OP + "\", \""
                + crtToken.tokenValue + "\", \"\")");
        ...
        final OpndSpec opndSpec = new OpndSpec(generateId(), PmEngine.PM_OP, crtToken.tokenValue, "NULL");
    ...
    private void semopNameOrFunctionCallInit() {
        ...
        int parent = ((Integer) parentStack.get(parentStack.size() - 1)).intValue();
    ...
    private void semopPolicyContInit() {
        ...
        traceSemact("set defaultType = " + PmEngine.PM_NODE_POL);
        defaultType = PmEngine.PM_NODE_POL;
    }
    ...
    private String semopRuleFinal() {
        traceSemop("semopRuleFinal");
        traceSemact("write rule spec to the active directory");
        String result = null;
        if (myEngine != null) {
            result = myEngine.generateRuleCode(ruleSpec, scriptId, lastRuleId);
        }
        if (result != null) {
    ...
    private void semopDeniedOp() {
        ...
        //  Since it doesn't appear to be used we input a placeholder
        // FIXME - Event Breaking Candidate
        OpndSpec opndSpec = new OpndSpec(generateId(), PmEngine.PM_OP, crtToken.tokenValue, "NULL");
    ...
    private void semopALabel() {
        ...
        traceSemact("create opndSpec(\"" + PmEngine.PM_LABEL + "\", \""
                + crtToken.tokenValue + "\", \"\")");
        ...
        //  Since it doesn't appear to be used we input a placeholder.
        //  Fixme event breaking candidate.
        OpndSpec opndSpec = new OpndSpec(generateId(), PmEngine.PM_LABEL, crtToken.tokenValue, "NULL");
        List<OpndSpec> opndSpecList = new ArrayList<OpndSpec>(1);
        opndSpecList.add(opndSpec);
    ...
    private void printParentStack() {
        ...
        for (int i = 0; i < parentStack.size(); i++) {
            int parent = ((Integer) parentStack.get(i)).intValue();
