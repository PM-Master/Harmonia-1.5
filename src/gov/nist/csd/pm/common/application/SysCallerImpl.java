...
public class SysCallerImpl implements SysCaller  {
    ...
    public List<String[]> getPosMembersOf(String sLabel, String sId, String sType,
            String sPresType) {
        System.out.println("GetPosMembersOf(label=" + sLabel + ", id=" + sId
                + ", type=" + sType);
        sLastError = null;
        try {
            Packet cmd = makeCmd(GET_POS_MEMBERS_OF, sSessionId, sLabel, sId,
                    sType, sPresType);
            Packet res = simClient.sendReceive(cmd, null);
            if (res == null) {
                sLastError = "Null result from getPosMembersOf()!";
                return null;
            }
            if (res.hasError()) {
   ...
   public Map<String, String> getPropertiesFor(String objId){
    ...
    public String createObject3(String sObjName, String sObjClass,
        ...
        try {
            ...
            Packet res = simClient.sendReceive(cmd, null);
            if (res.hasError()) {
                sLastError = "Error in createObject3: " + res.getErrorMessage();
                return null;
            }
            if (res.size() <= 0) {
