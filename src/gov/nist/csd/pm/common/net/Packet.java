...
public class Packet {
	 ...
	 * @uml.associationEnd  multiplicity="(0 -1)" elementType="gov.nist.csd.pm.common.net.Item"
	 */
	private ArrayList<Item> itemsArray = null;

	/**
	 * The payload length of this packet (i.e., the length of all Item objects in this packet.
	 * @uml.property  name="payloadLength"
	 */
	private int payloadLength = 0;

	/**
	 * Indicates that a CMD_CODE item has already been added. Note that only one command code item per packet is permitted.
	 * @uml.property  name="commandSet"
	 */
	private boolean commandSet = false;

	/**
	 * Indicates that a RESPONSE_TEXT item has been added. Note that no error or file items are permitted with a text response.
	 * @uml.property  name="responseTextSet"
	 */
	private boolean responseTextSet = false;

	/**
	 * Indicates that a RESPONSE_ERROR item has already been added. Note that only one error message item per packet is permitted.
	 * @uml.property  name="responseErrorSet"
	 */
	private boolean responseErrorSet = false;
        
	/**
	 * Indicates that a BINARY item has already been added. Note that only one file item per packet is permitted.
	...
	/**
	 * Indicates that the packet successfully received a binary stream and wrote the data to the given output stream.
	...
	public Packet(byte[] bytes) {
		...
		System.arraycopy(bytes, offset, payloadLengthBytes, 0, LENGTH_FIELD_SIZE);
	...
	/**
	 * Add an item without a value.  This method is convenient for some
	 * item types that do not have an associated value (e.g., RESPONSE_SUCCESS).
	 * However, caution should be used when using this method since most item types
	 * require an associated value (e.g., CMD_CODE).  Using this method
	 * for item types that require an associated value will lead to unexpected
	 * results.  This method automatically sets an empty
	 * string "" as the value.
	 * 
	 * @param itemType
	...
	public void addItem(ItemType itemType, String value) throws PacketException {
		...
		if (itemType == ItemType.RESPONSE_DNR) {
			...
			if (value == null) value = ""; //Added by Gopi
	...
	/**
	 * Add a binary stream of size 'payloadLength'.  This method is intended
	 * for streams that contain a file or other binary payload.  This method
	 * should only be called by the PacketManager.
	 * 
	 * @param payloadLength
	...
	private void testItemType(ItemType itemType) throws PacketException {
		...
		if (itemType == ItemType.CMD_CODE) {
			...
			if (commandSet == false && 
					responseTextSet == false && 
					responseErrorSet == false && 
					binarySet == false) {

				commandSet = true;
				
			} else if (responseTextSet == true || 
	...
	public Iterator<String> toStringIterator(){
        return new Iterator<String>(){
            ...
            @Override
            public void remove() {
                throw new RuntimeException("remove not implemented in Packet.getStringIterable");
            }
        };
    }
        ...
        public String getStringValue(int i) {
          if (i < 0 || i >= itemsArray.size()) return null;
	 ...
	 * @param index The index of the Item block to retrieve.
	 * @return The indexed Item block from this packet.
	 * @throws PacketException if index is out of bounds.
	 */
	 ...
	 * @param index The index of the Item block.
	 * @return The type of the indexed Item block contained in this packet.
	 * @throws PacketException index is out of bounds.
	 */
	...
	/**
	 * Get the byte array value of the indexed Item block contained in this packet.
	 * @param index The index of the Item block.
	...
	public byte[] getBytes() {
		...
		System.arraycopy(payloadLengthBytes, 0, bytes, offset, LENGTH_FIELD_SIZE);
		offset += LENGTH_FIELD_SIZE;
	...
	/**
	 * Indicates if the packet contains a BINARY stream that
	 * was successfully received and written to the given output stream.
	...
	public boolean hasText() {
        ...
        public void print(boolean bDebug, String sDirection) {
          ...
          for (int i = 0; i < itemsArray.size(); i++) {
            ...
            if (v == null) System.out.println(sDirection + t.toPrefix());
            else System.out.println(sDirection + t.toPrefix() + " " + v);
          }
    ...
    public static  Packet successPacket() {
        Packet p = new Packet();
        try {
