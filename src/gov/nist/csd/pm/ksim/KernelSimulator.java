...
public class KernelSimulator {
    ...
    public static final int PM_DEFAULT_ENGINE_PORT = 8080;
    public static final int PM_DEFAULT_SIMULATOR_PORT = 8081;
    ...
    Set<PMCommand> passToEngineCommands = EnumSet.of(ADD_HOST_APP,ADD_PROP, GET_ENTITY_WITH_PROP, GET_POLICY_CLASSES, GET_SIMPLE_VOS_GRAPH,
            ...
            GET_VOS_ID_PROPERTIES, GET_K_STORE_PATHS, SET_K_STORE_PATHS, DELETE_SESSION, GET_HOST_APP_PATHS,
            SET_HOST_APP_PATHS, GET_HOST_REPOSITORY, REQUEST_PERMS, REF_MEDIATION, WOULD_OPEN_PREVENT_SAVE,
            ...
            GET_INBOX_MESSAGES, GET_OUTBOX_MESSAGES, GET_POS_MEMBERS_OF, GET_POS_CONTAINERS_OF, SET_STARTUPS,
            ...
            GET_USERS_OF, GET_MEMBERS_OF, GET_CONNECTOR, ADD_OATTR, GET_INSTALLED_APP_NAMES, CREATE_LINKED_OBJECTS);
    ...
    private Packet refMediation(String sSessId, String sObjName) {
        try {
            Packet cmd = makeCmd(REF_MEDIATION, sSessId, sObjName);
            Packet res = engineClient.sendReceive(cmd, null);
            return res;
        } catch (Exception e) {
            e.printStackTrace();
            return failurePacket(String.format(EXCEPTION_IN_S_S_ERROR_FMTMSG, REF_MEDIATION, e.getMessage()));
        }
    }

    private Packet requestPerms(String sSessId, String sObjName,
                                String sReqPerms) {
        try {
            Packet cmd = makeCmd(REQUEST_PERMS, sSessId, sObjName, sReqPerms);
            Packet res = engineClient.sendReceive(cmd, null);
            return res;
        } catch (Exception e) {
            e.printStackTrace();
            return failurePacket(String.format(EXCEPTION_IN_S_S_ERROR_FMTMSG, REQUEST_PERMS, e.getMessage()));
        }
    }
   ...
   // private Map<String, Map<File, InputStream>> _sessionFileStreams = Collections.synchronizedMap(new HashMap<String, Map<File, InputStream>>());
    ...
    public static void main(String[] args) {
        ...
        if (engport < 1024) {
            engport = PM_DEFAULT_ENGINE_PORT;
        }
        if (simport < 1024) {
