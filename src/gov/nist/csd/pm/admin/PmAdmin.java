...
public class PmAdmin extends JFrame implements ActionListener, TreeExpansionListener{
    ...
    /**
	 * @uml.property  name="sCurrentClient"
	 */
    ...
    /**
	 * @uml.property  name="objEditor"
	 * @uml.associationEnd  
	 */
    private ObjEditor objEditor = null;
    /**
	 * @uml.property  name="opsetEditor"
	 * @uml.associationEnd  
	 */
    ...
    /**
	 * @uml.property  name="asetEditor"
	 * @uml.associationEnd  
	 */
    private AsetEditor asetEditor = null;
    /**
	 * @uml.property  name="sacEditor"
	 * @uml.associationEnd  
	 */
    private SacEditor sacEditor = null;
    /**
	 * @uml.property  name="sessionViewer"
	 * @uml.associationEnd  
	 */
    private SessionViewer sessionViewer = null;
    /**
	 * @uml.property  name="denyEditor"
	 * @uml.associationEnd  
	 */
    ...
    /**
	 * @uml.property  name="taskEditor"
	 * @uml.associationEnd  
	 */
    private TaskEditor taskEditor = null;
    /**
	 * @uml.property  name="sconEditor"
	 * @uml.associationEnd  
	 */
    private SconEditor sconEditor = null;
    /**
	 * @uml.property  name="sconaEditor"
	 * @uml.associationEnd  
	 */
    private SconaEditor sconaEditor = null;
    /**
	 * @uml.property  name="emlAcctEditor"
	 * @uml.associationEnd  
	 */
    private EmlAcctEditor emlAcctEditor = null;
    /**
	 * @uml.property  name="propertyEditor"
	 * @uml.associationEnd  
	 */
    ...
    /**
	 * @uml.property  name="childDelegate"
	 * @uml.associationEnd  
	 */
    ...
    public PmAdmin(String title, boolean debug, String host, int port, String session,
        ...
        menuItem = new JMenuItem("Attribute sets...");
        menuItem.addActionListener(this);
        menu.add(menuItem);

        menuItem = new JMenuItem("SACs...");
        menuItem.addActionListener(this);
        menu.add(menuItem);
        ...
        menuItem = new JMenuItem("Static constraints...");
        menuItem.addActionListener(this);
        menu.add(menuItem);

        menuItem = new JMenuItem("Static constraints for attributes...");
        menuItem.addActionListener(this);
        menu.add(menuItem);
        ...
        cmdEditor.pack();
        asetEditor = new AsetEditor(this, sslClient);
        asetEditor.pack();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (sCommand.equals("Import configuration...")) {
            ...
            doUsers();
        } else if (sCommand.equals("Attribute sets...")) {
            doAttributeSets();
        } else if (sCommand.equals("SACs...")) {
            doSACs();
            ...
            doInfo();
        } else if (sCommand.equals("Request perms on node...")) {
            doRequestPermsOnPmEntity();
        } else if (sCommand.equals("Request perms")) {
            doRequestPermsOnObj();
    ...
    private void doImportCfg() {
        ...
        try {
            ...
            for (int i = 0; i < delayedLines.size(); i++) {
                sLine = (String) delayedLines.get(i);
                Packet cmd = makeCmd("interpretCmd", sLine);
                Packet res = sslClient.sendReceive(cmd, null);
                if (res.hasError()) {
                    JOptionPane.showMessageDialog(this, res.getErrorMessage());
 ...
 // This method sends all config commands in a block.
    ...
    private void doDeleteObjects() {
        try {
            // Prepare engine command.
            Packet cmd = makeCmd("deleteContainerObjects", rightClickedNode.getName());
            Packet res = sslClient.sendReceive(cmd, null);
            if (res == null) {
    ...
    private void doDacConfinement() {
        try {
            ...
            if (res.hasError()) {
                JOptionPane.showMessageDialog(this, "No DAC policy found!");
                return;
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
    ...
    // Called when the admin selects the "Attribute Sets..." menu item.
    private void doAttributeSets() {
        asetEditor.prepare();
        asetEditor.setVisible(true);

        // Return here
    }

    // Subject Activation Constraints.
    // Called when the admin selects the "SACs..." menu item.
    private void doSACs() {
        sacEditor.prepare();
        sacEditor.setVisible(true);
    ...
    // of the types of x, y.
    @SuppressWarnings("CallToThreadDumpStack")
    private List<String[]> getMembersOf(String sType, String sId, String sLabel) {
        Packet res = null;
        try {
    ...
    // Request permissions on the clicked graph node/PM entity seen as an object.
    // Send the node id and type to the engine; the engine will look for a
    // virtual object mapped to this node and compute the permissions on that
    // virtual object.
    @SuppressWarnings("CallToThreadDumpStack")
    private void doRequestPermsOnPmEntity() {
        String sReqPerms = JOptionPane.showInputDialog(null, "Please enter requested permissions, separated by commas:");
        if (sReqPerms == null) {
            return;
        }

        System.out.println("Request perms " + sReqPerms + " on " + rightClickedNode.getId() + ":" + rightClickedNode.getName() + ":" + rightClickedNode.getType());

        Packet res = null;
        try {
            Packet cmd = makeCmd("requestPermsOnPmEntity", sessionId, rightClickedNode.getId(),
                    rightClickedNode.getType(), sReqPerms);
            res = sslClient.sendReceive(cmd, null);
            if (res == null) {
                JOptionPane.showMessageDialog(this, "Undetermined error; null result returned");
                return;
